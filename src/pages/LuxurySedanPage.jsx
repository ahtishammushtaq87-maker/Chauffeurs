...
export default function LuxurySedanPage() {
  ...
  const sedan = luxurySedanFleet.card;
    ...
    <>
      ...
      <section id="quote" className="relative flex items-center overflow-hidden border-b border-ink-border bg-ink">
        <div className="absolute inset-0">
          <PlaceholderImage src={luxurySedanImages.heroImg} alt="Luxury Mercedes-Benz sedan with chauffeur service in Nashville, TN" />
        </div>
        ...
        <div className="relative z-10 flex w-full flex-col items-start gap-6 px-5 py-10 sm:gap-10 sm:px-10 sm:py-14 md:px-16 lg:flex-row lg:items-center lg:justify-between lg:gap-8 lg:px-20">
          <div className="flex-shrink-0 rounded-lg bg-black/55 p-5 sm:p-6 lg:max-w-lg">
            <span className="eyebrow">{luxurySedanHero.eyebrow}</span>
            <h1 className="font-serif text-[2rem] leading-[1.15] font-medium text-ink-fg sm:text-5xl md:text-[56px]">
              {luxurySedanHero.h1}
            </h1>
            <p className="mt-3 font-serif text-lg text-gold italic sm:text-xl">{luxurySedanHero.tagline}</p>
            <p className="mt-5 text-[15px] leading-relaxed text-ink-fg-muted">{luxurySedanHero.subtext}</p>
            <TrustBadges />
            <div className="mt-7 flex flex-wrap gap-3 sm:mt-9 sm:gap-4">
              ...
              <a href="#quote" className="btn btn-outline border-ink-border-strong text-ink-fg hover:border-gold-light hover:text-gold-light">
                Check Sedan Availability &amp; Pricing
              </a>
            </div>
          </div>
          ...
          <div className="w-full max-w-md lg:flex-shrink-0">
            <QuoteForm
              heading={<>Get a Luxury Sedan <span className="text-gold">Quote</span></>}
              submitLabel="Check Sedan Availability & Pricing"
              defaultVehicle="Luxury Sedan (up to 3 passengers)"
            />
          </div>
        </div>
      </section>
      ...
      <section className="px-6 py-20 md:px-16 lg:px-24">
        <div className="mx-auto grid max-w-(--breakpoint-xl) grid-cols-1 items-center gap-12 lg:grid-cols-2">
          <div className="relative order-2 aspect-[4/3] overflow-hidden rounded-xl border border-border lg:order-1">
            <PlaceholderImage src={luxurySedanImages.featuresImg} alt="Interior of a luxury Mercedes sedan for private Nashville transportation" />
          </div>
          <div className="order-1 lg:order-2">
            ...
            <p className="mt-5 text-[15px] leading-relaxed text-text-muted">{luxurySedanIntro}</p>
      ...
      <section className="border-y border-border bg-panel/40 px-6 py-16 md:px-16 md:py-20 lg:px-24">
        <div className="mx-auto max-w-(--breakpoint-xl)">
          <div className="mb-12 max-w-2xl">
            <span className="eyebrow">{luxurySedanFleet.eyebrow}</span>
            <h2 className="font-serif text-3xl leading-tight font-medium text-text sm:text-4xl">
              {luxurySedanFleet.headingLead} <span className="text-gold">{luxurySedanFleet.headingAccent}</span>
            </h2>
          </div>
          ...
          <div className="grid grid-cols-1 items-center gap-6 overflow-hidden rounded-2xl border border-border bg-bg lg:grid-cols-2 lg:gap-10">
            <div className="aspect-[16/10] max-h-[360px] w-full overflow-hidden">
              <PlaceholderImage src={sedan.image} alt="Mercedes-Benz S-Class luxury sedan in Nashville, TN" />
            </div>
            <div className="p-7 pt-0 sm:p-10 lg:pt-10">
              <h3 className="font-serif text-2xl leading-tight font-medium text-text sm:text-3xl">{sedan.name}</h3>
              <p className="mt-2 text-[13px] font-semibold tracking-[1.2px] text-gold uppercase">{sedan.capacity}</p>
              <p className="mt-4 text-[15px] leading-relaxed text-text-muted">{sedan.desc}</p>
              <ul className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2">
                {sedan.points.map((point) => (
      ...
      {/* Best uses — Airport, Business, Private Travel */}
      <section className="px-6 py-16 md:px-16 md:py-20 lg:px-24">
        <div className="mx-auto max-w-(--breakpoint-xl)">
          <div className="mb-12 max-w-2xl">
            <span className="eyebrow">{luxurySedanUses.eyebrow}</span>
            <h2 className="font-serif text-3xl leading-tight font-medium text-text sm:text-4xl">
              {luxurySedanUses.heading}
            </h2>
          </div>

          <div className="grid grid-cols-1 gap-6 md:grid-cols-3">
            {luxurySedanUses.items.map((item) => (
      ...
      <section className="border-y border-border bg-panel/40 px-6 py-16 md:px-16 md:py-20 lg:px-24">
        <div className="mx-auto grid max-w-(--breakpoint-xl) grid-cols-1 items-center gap-12 lg:grid-cols-2">
          <div className="relative order-2 aspect-[4/3] overflow-hidden rounded-xl border border-border lg:order-1">
            <PlaceholderImage src={luxurySedanImages.includesImg} alt="Chauffeur with a luxury Mercedes sedan in Nashville" />
          </div>
          <div className="order-1 lg:order-2">
            <span className="eyebrow">{luxurySedanIncludes.eyebrow}</span>
            <h2 className="font-serif text-3xl leading-tight font-medium text-text md:text-4xl">
              {luxurySedanIncludes.heading}
            </h2>
            <ShowMore collapsedHeight="7rem" fadeFrom="from-panel">
              <p className="mt-5 text-[15px] leading-relaxed text-text-muted">{luxurySedanIncludes.body}</p>
            </ShowMore>
            <ul className="mt-7 grid grid-cols-1 gap-3.5 sm:grid-cols-2">
              {luxurySedanIncludes.features.map((line) => (
                <li key={line} className="flex items-start gap-3 text-sm text-text-muted">
                  <CheckCircleIcon width={20} height={20} className="mt-0.5 flex-shrink-0 text-gold" />
                  {line}
                </li>
        ...
        <div className="mx-auto max-w-3xl">
          <div className="mb-10 text-center">
            ...
            <h2 className="font-serif text-3xl font-medium text-text md:text-4xl">
              Nashville Luxury Sedan Service FAQs
            </h2>
          </div>
          <FaqAccordion items={luxurySedanFaqs.map(({ q, a }) => ({ q, a }))} />
        </div>
      </section>
      ...
      <section className="border-t border-border px-6 py-20 md:px-16 lg:px-24">
        <div className="mx-auto flex max-w-(--breakpoint-xl) flex-col items-center gap-5 text-center">
          <h2 className="font-serif text-3xl font-medium text-text md:text-4xl">Get Your Nashville Luxury Sedan Quote</h2>
          <p className="max-w-xl text-[15px] text-text-muted">
            Share your date, pickup time, passenger count, luggage, pickup location, and destination to check sedan
            availability and receive clear pricing.
          </p>
          <div className="flex flex-wrap justify-center gap-4">
            <a href="#quote" className="btn btn-gold">
              Check Sedan Availability
            </a>
