import { XIcon, LinkedinIcon, PinterestIcon } from "./Icons";

// Monochrome glyphs: the shared FacebookIcon/WhatsApp marks carry their own brand
// colors, which fight the gold tiles here. These inherit currentColor instead.
function FacebookGlyph(props) {
  return (
    <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true" {...props}>
      <path d="M15.4 12.5h-2V21h-3v-8.5H8.9V10h1.5V8.5c0-1.9 1-3 3.2-3h2V8h-1.3c-.5 0-.9.4-.9.9V10h2.2l-.2 2.5z" />
    </svg>
  );
}
...
function WhatsappGlyph(props) {
  return (
    <svg width="16" height="16" viewBox="0 0 32 32" fill="currentColor" aria-hidden="true" {...props}>
...
// Share buttons for a single article. Renders a row on mobile and a vertical
// rail on desktop — the parent decides placement.
export default function ShareRail({ url, title, className = "" }) {
  const u = encodeURIComponent(url);
  const t = encodeURIComponent(title);

  const links = [
    { label: "Share on Facebook", href: `https://www.facebook.com/sharer/sharer.php?u=${u}`, Icon: FacebookGlyph },
    ...
    { label: "Share on WhatsApp", href: `https://wa.me/?text=${t}%20${u}`, Icon: WhatsappGlyph },
    { label: "Share on Pinterest", href: `https://pinterest.com/pin/create/button/?url=${u}&description=${t}`, Icon: PinterestIcon },
  ];
    ...
    <div className={`flex gap-2 ${className}`}>
      {links.map(({ label, href, Icon }) => (
        <a
          ...
          aria-label={label}
          title={label}
          className="flex h-10 w-10 flex-shrink-0 items-center justify-center rounded-sm bg-gold text-[#17140d] transition-colors duration-200 hover:bg-gold-light"
        >
